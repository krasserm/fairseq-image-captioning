...
then
    ...
    echo "Generate validation dataset files ..."
    fairseq-preprocess --source-lang en --only-source --validpref $BPE_PREF --destdir $OUT_DIR --srcdict $OUT_DIR/dict.en.txt

    mv $OUT_DIR/valid.en-None.en.bin $OUT_DIR/valid-captions.en.bin
    mv $OUT_DIR/valid.en-None.en.idx $OUT_DIR/valid-captions.en.idx
fi
